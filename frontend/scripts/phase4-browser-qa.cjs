/**
 * Phase 4 real-browser responsive QA (Edge headless via puppeteer-core).
 * Patient Folder + clinical record screens at required viewports.
 */
...
async function main() {
  ...
  try {
    // Doctor — Patient Folders + folder detail + consultation view
    {
      ...
      try {
        ...
        if (ctx.patientId) {
          ...
          Object.assign(
            ...
            await probePage(
              page,
              `/doctor/records?patient=${ctx.patientId}&section=timeline`
            )
          );
        }
    ...
    // Patient — records folder
    {
      ...
      try {
        ...
        const login = await clinicLogin(page, 'patient@ecdoctor.co.za');
        ...
        if (recordId) {
          Object.assign(
            roleResult.pages,
            await probePage(page, `/patient/records/view/${recordId}`)
          );
        }
      } catch (err) {
        roleResult.error = err instanceof Error ? err.message : String(err);
    ...
    // Reception — patient detail (operational only)
    {
  ...
  const outPath = path.join(__dirname, '..', '..', 'PHASE4_BROWSER_QA.json');
